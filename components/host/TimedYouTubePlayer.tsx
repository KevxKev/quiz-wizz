...
declare global {
  interface Window {
    ...
    __quizWizzYouTubeApiPromise?: Promise<void>;
  }
}
...
function loadYouTubeIframeApi(logEvent: (message: string, details?: unknown) => void): Promise<void> {
  ...
  if (window.__quizWizzYouTubeApiPromise) {
    return window.__quizWizzYouTubeApiPromise;
  }

  window.__quizWizzYouTubeApiPromise = new Promise<void>((resolve, reject) => {
  ...
  function TimedYouTubePlayer(
    ...
    const logEvent = useCallback((message: string, details?: unknown) => {
      if (details !== undefined) {
        console.info(`[Quiz Wizz][YouTube] ${message}`, details);
        return;
      }

      console.info(`[Quiz Wizz][YouTube] ${message}`);
    }, []);
    ...
    const runPlaybackSequence = useCallback(
      (reason: string) => {
        ...
        player.stopVideo();
        logEvent("Loading target video for deterministic playback.", {
          videoId,
          startSeconds,
          endSeconds,
          reason,
        ...
        playbackSequenceTimeoutRef.current = window.setTimeout(() => {
          ...
          logEvent("seekTo called with stored round start.", { seconds: startSeconds, reason });
          activePlayer.seekTo(startSeconds, true);

          logEvent("playVideo called.", { reason });
          ...
          window.setTimeout(() => {
            const verifyPlayer = playerRef.current;

            if (!verifyPlayer) {
              return;
            }

            const rawTime = verifyPlayer.getCurrentTime?.();
            const currentTime = typeof rawTime === "number" && Number.isFinite(rawTime)
              ? Number(rawTime.toFixed(2))
              : startSeconds;

            if (Math.abs(currentTime - startSeconds) > 1.5 && currentTime < endSeconds) {
              logEvent("Playback drift detected after autoplay; re-seeking.", {
                currentTime,
                targetStart: startSeconds,
              });
              verifyPlayer.seekTo(startSeconds, true);
              verifyPlayer.playVideo();
            }
          }, 350);
        }, 180);
      },
    ...
    useEffect(() => {
      ...
      loadYouTubeIframeApi(logEvent)
        ...
        .catch((error: unknown) => {
          ...
          console.error("[Quiz Wizz][YouTube] API script load failure", error);
          updateDebugState((current) => ({
            ...current,
            apiStatus: "failed",
            errorFired: true,
      ...
      const player = new window.YT!.Player(playerMountRef.current, {
        ...
        events: {
          ...
          onError: (event) => {
            ...
            console.error("[Quiz Wizz][YouTube] Player error", { errorCode });
            clearStopMonitor();
            updateDebugState((current) => ({
              ...current,
              errorFired: true,
    ...
    useEffect(() => {
      if (!playerRef.current || !playerReadyRef.current || !videoId || validationMessage) {
        return;
      }
      ...
      >
        {!naked ? (
          <div className="mb-3 flex justify-end">
            <span className="rounded-full px-3 py-1 text-sm font-medium" style={{ background: "rgba(5,3,18,0.80)", border: "1px solid rgba(201,162,39,0.30)", color: "var(--oly-gold-bright)" }}>{getPlaybackModeLabel(playbackMode)}</span>
          </div>
        ) : null}
        ...
        <div className={naked ? "relative h-full w-full overflow-hidden bg-black" : "relative aspect-video overflow-hidden rounded-2xl bg-black"}>
          ...
          {playbackMode === "audio-only" && debugState.readyFired ? (
            <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center text-center text-slate-100" style={{ background: "var(--oly-night-base)" }}>
              <div className="rounded-full px-4 py-1 text-xs font-semibold uppercase tracking-[0.25em]" style={{ border: "1px solid rgba(201,162,39,0.30)", background: "rgba(201,162,39,0.10)", color: "var(--oly-gold-bright)" }}>
                Audio only mode
              </div>
