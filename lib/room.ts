...
export const DEFAULT_REVEALED_REPLAY_SECONDS = 15;
...
export function selectQuizEntryForRound(entries: QuizEntry[], roundNumber: number): QuizEntry | null {
  ...
  // Pick a random entry, avoiding the most recently used ones where possible.
  // We seed with roundNumber so re-renders are stable within a round but
  // each round is independently random.
...
export function getPhaseDurationSeconds(
  ...
  switch (phase) {
    ...
    case "revealed":
      return DEFAULT_REVEALED_REPLAY_SECONDS;
    case "leaderboard":
...
export function buildLeaderboard(players: RoomPlayer[], rounds: Round[], answers: RoundAnswer[]): LeaderboardEntry[] {
  ...
  for (const answer of answers) {
    ...
    if (round.correct_answer && answer.answer_text === round.correct_answer) {
      ...
      const pts = Math.round(computeAnswerScore(answer.answered_after_ms ?? null, totalWindowMs));
